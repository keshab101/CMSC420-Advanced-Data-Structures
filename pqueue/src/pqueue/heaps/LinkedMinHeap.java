...
public class LinkedMinHeap<T extends Comparable<T>> implements MinHeap<T> {
	...
	public void insertHelper(T element, MinHeapNode node) {
		
		MinHeapNode elem = new MinHeapNode(element);
		if(node.lChild == null) {
			node.lChild = elem;		
			...
			if(node.lChild.lChild == null || node.lChild.rChild == null) {
				insertHelper(element, node.lChild);
			} else if(node.rChild.lChild == null || node.rChild.rChild == null) {
				insertHelper(element, node.rChild);
			} else {
				insertHelper(element, node.lChild);
			}
		}	
